...
export async function renameSkill(input: RenameInput): Promise<OpResult> {
  try {
    ...
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(input.newName) || input.newName.length > 64) {
      throw new SkilletError({ message: 'Use up to 64 lowercase letters, numbers, and hyphens.', method: 'renameSkill', service: SERVICE, error: null, code: 'VALIDATION', status: 400 });
    }
    ...
    const linkedSource = canonical.kind === 'symlink';
    const sourcePath = linkedSource ? await realpath(canonical.absPath) : canonical.absPath;
    await assertRealDir(sourcePath);
    ...
    if (linkedSource) {
      steps.push(step('copyDir', sourcePath, newCanonicalPath, `create renamed local copy ${input.newName}`));
      steps.push(step('unlink', '', canonical.absPath, 'remove previous local link'));
      inverse.push(step('removeDir', '', newCanonicalPath, 'remove renamed copy'));
      inverse.push(symlinkStep(sourcePath, canonical.absPath, canonical.scope === 'project', 'restore original link'));
    } else {
    ...
    for (const instance of target.instances) {
      ...
      await assertInsideRoots([instance.absPath, newLinkPath]);
      await assertFree(newLinkPath);
      const originalTarget = await realpath(instance.absPath);
      ...
      inverse.push(symlinkStep(originalTarget, instance.absPath, useRelative, 'restore original symlink'));
    }
