...
import { copySkill, installEverywhere, linkSkill, moveSkill, unlinkInstance } from '../hub/linking';
import type { LinkTarget } from '../hub/linking';
import type { OpResult } from '../hub/steps.types';
import { trashSkill } from '../hub/trash';
import { setTrigger, setSkillInvocation } from '../hub/trigger';
...
skillsRoute.post('/:id/install-all', async (c) => {
  try {
    const body = await c.req.json<{ dryRun: boolean }>();
    return c.json(await finish(await installEverywhere({ skillId: c.req.param('id'), dryRun: body.dryRun })));
  } catch (error) { wrap('installEverywhere', error); }
});

skillsRoute.post('/:id/remove-installation', async (c) => {
  try {
    const id = c.req.param('id');
    const body = await c.req.json<{ instanceId: string; dryRun: boolean }>();
    const instance = getIndex().skills.find((skill) => skill.id === id)?.instances.find((entry) => entry.id === body.instanceId);
    if (!instance) return c.json({ message: 'Installation not found' }, 404);
    const result = instance.kind === 'symlink'
      ? await unlinkInstance({ skillId: id, ...body })
      : await trashSkill({ skillId: id, ...body });
    return c.json(await finish(result));
  } catch (error) { wrap('removeInstallation', error); }
});

skillsRoute.post('/:id/invocation', async (c) => {
  try {
    const body = await c.req.json<{ automatic: boolean; dryRun: boolean }>();
    if (typeof body.automatic !== 'boolean') return c.json({ message: 'Choose a usage mode' }, 400);
    return c.json(await finish(await setSkillInvocation({ skillId: c.req.param('id'), ...body })));
  } catch (error) { wrap('setSkillInvocation', error); }
});
