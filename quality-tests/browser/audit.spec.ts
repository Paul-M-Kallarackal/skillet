...
test('share copies a portable skill message and provides selectable text if clipboard is blocked', async ({ page }) => {
  ...
  await expect(page.getByRole('status')).toContainText('Copied. Paste into Slack');
  const text = await page.evaluate(() => sessionStorage.getItem('shared-skill'));
  expect(text).toContain('ChatGPT, Claude');
  ...
  await expect(dialog.getByRole('textbox', { name: 'Message to share' })).toHaveValue(text!);
  await expectNoSeriousAccessibilityViolations(page);
});
